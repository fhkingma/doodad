...
from .utils import hash_file, call_and_wait, call, CommandBuilder
...
class DockerMode(LaunchMode):
    def __init__(self, image='ubuntu:16.04', gpu=False, privileged=False):
    ...
    def get_docker_cmd(self, main_cmd, extra_args='', use_tty=True, verbose=True , pythonpath=None, pre_cmd=None, post_cmd=None,
            checkpoint=False, no_root=False, tensorboard=False):
...
class LocalDocker(DockerMode):
    ...
    def launch_command(self, cmd, mount_points=None, dry=False, verbose=False, tensorboard=False):
        mnt_args = ''
        py_path = []
        for mount in mount_points:
            if isinstance(mount, MountLocal):
...
class SSHDocker(DockerMode):
    ...
    def launch_command(self, main_cmd, mount_points=None, dry=False, verbose=False, tensorboard=False):
        ...
        if self.checkpoint and self.checkpoint.restore:
            ...
            docker_cmd = self.get_docker_cmd(main_cmd, use_tty=False, extra_args=mnt_args, pythonpath=py_path, tensorboard=tensorboard)
